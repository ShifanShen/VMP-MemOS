...
CANDIDATE_COUNT="${CANDIDATE_COUNT:-${VMP_V52_CANDIDATE_COUNT:-30}}"
OUTPUT_TOP_K="${OUTPUT_TOP_K:-5}"
PROTECTED_TOP_N="${PROTECTED_TOP_N:-${VMP_V52_PROTECTED_TOP_N:-4}}"
MAX_CANDIDATE_CHARS="${MAX_CANDIDATE_CHARS:-${VMP_V52_MAX_CANDIDATE_CHARS:-1200}}"
MAX_EXCERPT_TURNS="${MAX_EXCERPT_TURNS:-4}"
RERANK_MAX_TOKENS="${RERANK_MAX_TOKENS:-512}"
RERANK_RESUME="${RERANK_RESUME:-0}"
DEV_CANDIDATE_RUN_ID="${DEV_CANDIDATE_RUN_ID:-lme_dev_vmp_v52_candidates_seed42}"
DEV_RERANK_RUN_ID="${DEV_RERANK_RUN_ID:-lme_dev_vmp_v52_rerank_seed42}"
TEST_CANDIDATE_RUN_ID="${TEST_CANDIDATE_RUN_ID:-lme_test_vmp_v52_candidates_seed42}"
TEST_RERANK_RUN_ID="${TEST_RERANK_RUN_ID:-lme_test_vmp_v52_rerank_seed42}"
...
GATE_RECEIPT="${GATE_RECEIPT:-outputs/longmemeval/gates/vmp_v52_seed42_dev_pass.json}"
LOG_DIR="${LOG_DIR:-outputs/longmemeval/logs}"
LOG_PATH="${LOG_PATH:-${LOG_DIR}/vmp_v52_${STAGE:-missing_stage}.log}"
TABLE_DIR="${TABLE_DIR:-outputs/longmemeval/tables/${TEST_RERANK_RUN_ID}}"
RUN_QA="${RUN_QA:-0}"

MIN_DEV_RECALL_ALL_5="${MIN_DEV_RECALL_ALL_5:-0.91}"
MIN_DEV_DELTA_VS_RAW_V5="${MIN_DEV_DELTA_VS_RAW_V5:-0.005}"
MIN_DEV_DELTA_VS_SHARED_V43="${MIN_DEV_DELTA_VS_SHARED_V43:-0.005}"
MIN_DEV_MACRO_DELTA_VS_SHARED_V43="${MIN_DEV_MACRO_DELTA_VS_SHARED_V43:-0.0}"
MAX_DEV_WORST_TYPE_REGRESSION_VS_SHARED_V43="${MAX_DEV_WORST_TYPE_REGRESSION_VS_SHARED_V43:-0.03}"
...
on_exit() {
  ...
  if [[ "${exit_code}" -eq 0 ]]; then
    log_stage "VMP-v5.2 stage ${STAGE:-unset} completed successfully."
  else
    log_stage "VMP-v5.2 stage ${STAGE:-unset} failed or was interrupted (exit_code=${exit_code})."
  fi
}
trap on_exit EXIT
...
run_rerank() {
  ...
  python scripts/run_longmemeval_rerank.py \
    ...
    --protected-top-n "${PROTECTED_TOP_N}" \
...
check_dev_gate() {
  python scripts/check_vmp_v52_gate.py \
    ...
    --min-macro-delta-vs-shared-v43 "${MIN_DEV_MACRO_DELTA_VS_SHARED_V43}" \
    --max-worst-type-regression-vs-shared-v43 \
      "${MAX_DEV_WORST_TYPE_REGRESSION_VS_SHARED_V43}" \
...
case "${STAGE}" in
  dev_candidates)
    log_stage "Stage 1/4: creating split and V4.3/V5 Dev candidate pools."
    log_stage "Keep vLLM stopped; this stage uses BGE-M3 on ${EMBEDDING_DEVICE}."
    create_split
    run_candidates dev "${DEV_CANDIDATE_RUN_ID}"
    ;;
  dev_rerank)
    log_stage "Stage 2/4: shared local-vLLM reranking on Dev only."
    log_stage "source=${DEV_CANDIDATE_RUN} model=${VMP_LLM_MODEL}"
    log_stage "candidate_count=${CANDIDATE_COUNT} protected_top_n=${PROTECTED_TOP_N}"
    run_rerank "${DEV_CANDIDATE_RUN}" "${DEV_RERANK_RUN_ID}"
    log_stage "Enforcing V5.2 Dev-only quality gate."
    check_dev_gate
    ;;
  test_candidates)
    log_stage "Stage 3/4: rechecking the sealed Dev gate before Test retrieval."
    check_dev_gate
    log_stage "Generating frozen Test candidates. Stop vLLM before this BGE-M3 stage."
    create_split
    run_candidates test "${TEST_CANDIDATE_RUN_ID}"
    ;;
  test_rerank)
    log_stage "Stage 4/4: rechecking Dev gate before sealed Test reranking."
    check_dev_gate
    log_stage "Applying the same local-vLLM policy to every Test method."
...
print(",".join(f"{method}__vllm_rerank" for method in methods if method))
PY
    )"
    if [[ "${RUN_QA}" == "1" ]]; then
      log_stage "Generating QA with the same local vLLM after reranking."
