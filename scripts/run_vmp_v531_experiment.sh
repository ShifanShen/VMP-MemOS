...
BOUNDARY_PROMPT_VERSION="${BOUNDARY_PROMPT_VERSION:-${VMP_V531_BOUNDARY_PROMPT_VERSION:-vmp_v531_symbolic_boundary_v1}}"
...
DEV_CANDIDATE_RUN_ID="${DEV_CANDIDATE_RUN_ID:-${VMP_V531_DEV_CANDIDATE_RUN_ID:-lme_dev_vmp_v53_candidates_seed42}}"
DEV_SELECTOR_RUN_ID="${DEV_SELECTOR_RUN_ID:-${VMP_V531_DEV_SELECTOR_RUN_ID:-lme_dev_vmp_v53_rerank_seed42}}"
DEV_RERANK_RUN_ID="${DEV_RERANK_RUN_ID:-${VMP_V531_DEV_RERANK_RUN_ID:-lme_dev_vmp_v531_replay_seed42}}"
TEST_CANDIDATE_RUN_ID="${TEST_CANDIDATE_RUN_ID:-lme_test_vmp_v531_candidates_seed42}"
TEST_RERANK_RUN_ID="${TEST_RERANK_RUN_ID:-lme_test_vmp_v531_rerank_seed42}"
DEV_CANDIDATE_RUN="outputs/longmemeval/runs/${DEV_CANDIDATE_RUN_ID}"
DEV_SELECTOR_RUN="outputs/longmemeval/runs/${DEV_SELECTOR_RUN_ID}"
...
GATE_RECEIPT="${GATE_RECEIPT:-outputs/longmemeval/gates/vmp_v531_seed42_dev_pass.json}"
LOG_DIR="${LOG_DIR:-outputs/longmemeval/logs}"
LOG_PATH="${LOG_PATH:-${LOG_DIR}/vmp_v531_${STAGE}.log}"
...
on_exit() {
  ...
  if [[ "${exit_code}" -eq 0 ]]; then
    log_stage "VMP-v5.3.1 stage ${STAGE} completed successfully."
  elif [[ "${exit_code}" -eq 3 ]]; then
    log_stage "VMP-v5.3.1 run completed, but the strict quality gate failed (exit_code=3)."
  else
    log_stage "VMP-v5.3.1 stage ${STAGE} failed or was interrupted (exit_code=${exit_code})."
  fi
}
trap on_exit EXIT
...
run_rerank() {
  local source_run="$1"
  local run_id="$2"
  local selector_replay_run="${3:-}"
  local resume_args=()
  local replay_args=()
  if [[ "${RERANK_RESUME}" == "1" ]]; then
    resume_args=(--resume)
  fi
  if [[ -n "${selector_replay_run}" ]]; then
    replay_args=(--selector-replay-run "${selector_replay_run}")
  fi
...
case "${STAGE}" in
  dev_replay)
    ...
    require_completed_run "${DEV_SELECTOR_RUN}"
    log_stage "Stage 1/3: replaying the exact saved Dev selector and calling only V5.3.1 boundary."
    log_stage "No BGE embedding or first-stage selector requests will run."
    run_rerank "${DEV_CANDIDATE_RUN}" "${DEV_RERANK_RUN_ID}" "${DEV_SELECTOR_RUN}"
    log_stage "Enforcing the unchanged strict Dev-only quality gate."
    check_dev_gate
    ;;
  test_candidates)
    log_stage "Stage 2/3: rechecking the sealed V5.3.1 Dev gate."
    check_dev_gate
    ...
    ;;
  test_rerank)
    log_stage "Stage 3/3: rechecking the V5.3.1 Dev gate before sealed Test."
    check_dev_gate
    require_completed_run "${TEST_CANDIDATE_RUN}"
    log_stage "Running the unchanged selector plus symbolic boundary on sealed Test."
...
PY
    ...
    ;;
  *)
    echo "Unknown STAGE=${STAGE}. Expected dev_replay, test_candidates, or test_rerank." >&2
    exit 2
    ;;
esac
