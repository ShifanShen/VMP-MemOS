...
PINNED_OFFICIAL_LLM_RETRY_MAX_TOKENS="16384"
if [[ -n "${VMP_OFFICIAL_LLM_RETRY_MAX_TOKENS:-}" && \
      "${VMP_OFFICIAL_LLM_RETRY_MAX_TOKENS:-}" != "${PINNED_OFFICIAL_LLM_RETRY_MAX_TOKENS}" ]]; then
  echo "Refusing VMP_OFFICIAL_LLM_RETRY_MAX_TOKENS=${VMP_OFFICIAL_LLM_RETRY_MAX_TOKENS:-}; the paper protocol pins 16384. Unset the stale override." >&2
  exit 2
fi
