...
CANDIDATE_POOL_COUNT="${CANDIDATE_POOL_COUNT:-${VMP_V532_CANDIDATE_POOL_COUNT:-40}}"
CANDIDATE_COUNT="${CANDIDATE_COUNT:-${VMP_V532_CANDIDATE_COUNT:-30}}"
...
BOUNDARY_PROMPT_VERSION="${BOUNDARY_PROMPT_VERSION:-${VMP_V532_BOUNDARY_PROMPT_VERSION:-vmp_v532_atomic_set_boundary_v1}}"
MAX_CANDIDATE_CHARS="${MAX_CANDIDATE_CHARS:-1600}"
MAX_EXCERPT_TURNS="${MAX_EXCERPT_TURNS:-6}"
RERANK_MAX_TOKENS="${RERANK_MAX_TOKENS:-512}"
BOUNDARY_MAX_TOKENS="${BOUNDARY_MAX_TOKENS:-512}"
RERANK_RESUME="${RERANK_RESUME:-0}"

DEV_CANDIDATE_RUN_ID="${DEV_CANDIDATE_RUN_ID:-${VMP_V532_DEV_CANDIDATE_RUN_ID:-lme_dev_vmp_v532_candidates_seed42}}"
DEV_RERANK_RUN_ID="${DEV_RERANK_RUN_ID:-${VMP_V532_DEV_RERANK_RUN_ID:-lme_dev_vmp_v532_rerank_seed42}}"
TEST_CANDIDATE_RUN_ID="${TEST_CANDIDATE_RUN_ID:-lme_test_vmp_v532_candidates_seed42}"
TEST_RERANK_RUN_ID="${TEST_RERANK_RUN_ID:-lme_test_vmp_v532_rerank_seed42}"
...
GATE_RECEIPT="${GATE_RECEIPT:-outputs/longmemeval/gates/vmp_v532_seed42_dev_pass.json}"
LOG_DIR="${LOG_DIR:-outputs/longmemeval/logs}"
LOG_PATH="${LOG_PATH:-${LOG_DIR}/vmp_v532_${STAGE}.log}"
...
on_exit() {
  ...
  if [[ "${exit_code}" -eq 0 ]]; then
    log_stage "VMP-v5.3.2 stage ${STAGE} completed successfully."
  elif [[ "${exit_code}" -eq 3 ]]; then
    log_stage "VMP-v5.3.2 run completed, but the strict quality gate failed (exit_code=3)."
  else
    log_stage "VMP-v5.3.2 stage ${STAGE} failed or was interrupted (exit_code=${exit_code})."
  fi
}
trap on_exit EXIT
...
case "${STAGE}" in
  dev_candidates)
    ...
    log_stage "Stage 1/4: generating Dev candidate pools with BGE-M3."
    log_stage "Stop vLLM first. Retrieving ${CANDIDATE_POOL_COUNT} memories so dedup can backfill ${CANDIDATE_COUNT} unique sessions."
    create_split
    run_candidates dev "${DEV_CANDIDATE_RUN_ID}"
    ;;
  dev_rerank)
    require_completed_run "${DEV_CANDIDATE_RUN}"
    log_stage "Stage 2/4: running the shared selector plus V5.3.2 atomic-set boundary on Dev."
    log_stage "The strict preflight runs before the first LLM call and requires ${CANDIDATE_COUNT} unique sessions per sample."
    run_rerank "${DEV_CANDIDATE_RUN}" "${DEV_RERANK_RUN_ID}"
    log_stage "Enforcing the unchanged strict Dev-only quality gate."
    check_dev_gate
    ;;
  test_candidates)
    log_stage "Stage 3/4: rechecking the sealed V5.3.2 Dev gate."
    check_dev_gate
    if [[ ! -f "${V43_MODEL_PATH}" || ! -f "${V5_MODEL_PATH}" ]]; then
      echo "Missing frozen V4.3 or V5 model artifact." >&2
      exit 2
    ...
    ;;
  test_rerank)
    log_stage "Stage 4/4: rechecking the V5.3.2 Dev gate before sealed Test."
    check_dev_gate
    require_completed_run "${TEST_CANDIDATE_RUN}"
    log_stage "Running the shared selector plus atomic-set boundary on sealed Test."
