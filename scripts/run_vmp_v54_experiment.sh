...
SELECTOR_PROMPT_VERSION="${SELECTOR_PROMPT_VERSION:-vmp_v54_symbolic_span_selector_v1}"
...
# V5.4 changes only the LLM protocol, so Dev deliberately reuses the completed
# V5.3.2 candidate run. This prevents candidate drift from confounding the result.
DEV_CANDIDATE_RUN_ID="${DEV_CANDIDATE_RUN_ID:-lme_dev_vmp_v532_candidates_seed42}"
DEV_RERANK_RUN_ID="${DEV_RERANK_RUN_ID:-lme_dev_vmp_v54_rerank_seed42}"
TEST_CANDIDATE_RUN_ID="${TEST_CANDIDATE_RUN_ID:-lme_test_vmp_v54_candidates_seed42}"
TEST_RERANK_RUN_ID="${TEST_RERANK_RUN_ID:-lme_test_vmp_v54_rerank_seed42}"
...
GATE_RECEIPT="${GATE_RECEIPT:-outputs/longmemeval/gates/vmp_v54_seed42_dev_pass.json}"
LOG_DIR="${LOG_DIR:-outputs/longmemeval/logs}"
LOG_PATH="${LOG_PATH:-${LOG_DIR}/vmp_v54_${STAGE}.log}"
...
on_exit() {
  ...
  if [[ "${exit_code}" -eq 0 ]]; then
    log_stage "VMP-v5.4 stage ${STAGE} completed successfully."
  elif [[ "${exit_code}" -eq 3 ]]; then
    log_stage "VMP-v5.4 run completed, but the strict quality gate failed (exit_code=3)."
  else
    log_stage "VMP-v5.4 stage ${STAGE} failed or was interrupted (exit_code=${exit_code})."
  fi
}
trap on_exit EXIT
...
case "${STAGE}" in
  dev_rerank)
    ...
    log_stage "Stage 1/3: reranking the frozen V5.3.2 Dev candidates with V5.4."
    log_stage "Candidate labels and evidence spans are symbolic; real session IDs remain server-side."
    run_rerank "${DEV_CANDIDATE_RUN}" "${DEV_RERANK_RUN_ID}"
    log_stage "Enforcing the unchanged strict Dev-only quality gate."
    check_dev_gate
    ;;
  test_candidates)
    log_stage "Stage 2/3: rechecking the sealed V5.4 Dev gate."
    check_dev_gate
    ...
    ;;
  test_rerank)
    log_stage "Stage 3/3: rechecking the V5.4 Dev gate before sealed Test."
    check_dev_gate
    require_completed_run "${TEST_CANDIDATE_RUN}"
    log_stage "Running the shared V5.4 selector and boundary protocol on sealed Test."
