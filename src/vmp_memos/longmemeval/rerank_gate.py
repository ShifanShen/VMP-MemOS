"""Dev-only quality gate for the shared-vLLM VMP-v5.2 pipeline."""
...
def evaluate_v52_gate(
    candidate_run: str | Path,
    rerank_run: str | Path,
    *,
    vmp_method: str = "vmp_hierarchical",
    baseline_method: str = "vmp_tuned",
    min_recall_all_at_5: float = 0.91,
    min_delta_vs_raw_v5: float = 0.005,
    min_delta_vs_shared_v43: float = 0.005,
    min_macro_delta_vs_shared_v43: float = 0.0,
    max_worst_type_regression_vs_shared_v43: float = 0.03,
    max_parse_fallback_rate: float = 0.02,
    min_candidate_count: int = 30,
) -> dict[str, JsonValue]:
    """Evaluate V5.2 without reading Test labels or raw dataset records."""
    ...
    vmp_reranked_method = reranked_method_name(vmp_method)
    baseline_reranked_method = reranked_method_name(baseline_method)
    ...
    v52_recall = _metric(reranked_v5, "recall_all@5")
    ...
    v52_macro = _type_metric(reranked_v5, mode="macro")
    shared_v43_macro = _type_metric(reranked_v43, mode="macro")
    v52_worst = _type_metric(reranked_v5, mode="worst")
    shared_v43_worst = _type_metric(reranked_v43, mode="worst")
    delta_raw_v5 = v52_recall - raw_v5_recall
    delta_shared_v43 = v52_recall - shared_v43_recall
    delta_raw_v43 = v52_recall - raw_v43_recall
    macro_delta_shared_v43 = v52_macro - shared_v43_macro
    worst_delta_shared_v43 = v52_worst - shared_v43_worst
    ...
    prompt_version = reranked_v5.get("prompt_version")
    baseline_prompt_version = reranked_v43.get("prompt_version")
    ...
    processed_counts = [
        _integer(summary.get("processed_questions"))
        for summary in (raw_v5, raw_v43, reranked_v5, reranked_v43)
    ]
    evaluated_counts = [
        _integer(summary.get("evaluated_questions"))
        for summary in (raw_v5, raw_v43, reranked_v5, reranked_v43)
    ]
    ...
    checks = {
        ...
        "shared_prompt": (
            ...
            and baseline_prompt_version == prompt_version
        ),
        "candidate_depth": (
        ...
        "min_recall_all_at_5": v52_recall >= min_recall_all_at_5,
        "delta_vs_raw_v5": delta_raw_v5 >= min_delta_vs_raw_v5,
        "delta_vs_shared_v43": delta_shared_v43 >= min_delta_vs_shared_v43,
        "macro_non_regression": (macro_delta_shared_v43 >= min_macro_delta_vs_shared_v43),
        "worst_type_guard": (worst_delta_shared_v43 >= -max_worst_type_regression_vs_shared_v43),
    }
    ...
    return {
        ...
        "metrics": cast(
            JsonValue,
            {
                ...
                "v52_macro_type_recall_all@5": v52_macro,
                "shared_v43_macro_type_recall_all@5": shared_v43_macro,
                "macro_delta_vs_shared_v43": macro_delta_shared_v43,
                "v52_worst_type_recall_all@5": v52_worst,
                "shared_v43_worst_type_recall_all@5": shared_v43_worst,
                "worst_type_delta_vs_shared_v43": worst_delta_shared_v43,
            },
        ),
        "reranker": cast(
            JsonValue,
            {
                ...
                "protected_top_n": reranked_v5.get("protected_top_n"),
                "output_top_k": reranked_v5.get("output_top_k"),
                "parse_fallback_rate": fallback_rate,
                "baseline_parse_fallback_rate": baseline_fallback_rate,
                "recovered_questions": reranked_v5.get("recovered_questions"),
                "regressed_questions": reranked_v5.get("regressed_questions"),
                "candidate_oracle_recoverable_questions": reranked_v5.get(
                    "candidate_oracle_recoverable_questions"
                ),
        ...
        "required": cast(
            JsonValue,
            {
                ...
                "min_macro_delta_vs_shared_v43": (min_macro_delta_vs_shared_v43),
                "max_worst_type_regression_vs_shared_v43": (
                    max_worst_type_regression_vs_shared_v43
                ),
...
def write_gate_receipt(
    ...
    """Persist a receipt only after every Dev-only gate has passed."""

    if report.get("status") != "passed":
        raise ValueError("cannot write a gate receipt for a failed report")
...
def _type_metric(summary: dict[str, JsonValue], *, mode: str) -> float:
    ...
    for metrics in by_type.values():
        if isinstance(metrics, dict):
            values.append(_number(metrics.get("recall_all@5")))
    if not values:
        return 0.0
    if mode == "macro":
    ...
    if mode == "worst":
        return min(values)
    raise ValueError(f"unsupported type aggregation: {mode}")
